...
> 자바스크립트는 함수형 언어다. 왜냐하면 함수가 실행의 기본 모듈 단위이기 때문이다.

자바스크립트를 처음 배우는 사람에게 함수를 가르쳐야 한다면 기본적으로 알려줄 수 있는 것들.

## 함수는 일급 객체다

> 함수가 다른 자바스크립트 객체와 같은 지위를 가지며, 일반적인 다른 자바스크립트 객체처럼 취급할 수 있다는 것을 의미한다.
> 자바스크립트 닌자 비급 3장 p.38

자바스크립트 함수는 일급 객체이기 때문에 값으로 할당이 가능하다.(변수를 통해 함수를 참조 할 수 있다.)
리터럴 함수를 선언할 수 있다.
다른 함수에 매개 변수로 전달도 가능하다.
