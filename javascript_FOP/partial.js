...
!(function (G) {
  ...
  _.all = _.All = function (arg) {
    ...
    if (_.is_number(arg))
      return function f() {
        return ++count < arg
          ? (fns.push(_.pipe(arguments)), f)
          : _.all2.apply(this, [to_mr(arguments)].concat(fns));
      };
  ...
  _.is_equal = _.isEqual = function f(a, b) {
    return arguments.length == 1 ? _1(f, a) : eq(a, b);
  };
  ...
  function _push(arr, itemOrFunc) {
    arr.push.apply(
      arr,
      _.is_fn(itemOrFunc) ? [itemOrFunc(arr)] : _.rest(arguments),
    );
    return arr;
  }
  ...
  _.extend(_, {
    ...
    push: function (start, sel) {
      _push.apply(null, [_.sel(start, sel)].concat(_.rest(arguments, 2)));
      return start;
    },
    unshift: function (start, sel, item) {
      _unshift.apply(null, [_.sel(start, sel)].concat(_.rest(arguments, 2)));
      return start;
    },
  });
    ...
    return _.extend(_box, {
      ...
      extend2: function (el) {
        return _.extend2.apply(
          null,
          [_box_data, make_sel(el)].concat(_.rest(arguments)),
        );
      },
  ...
  function resovle(self, val) {
    try {
      if (val && val instanceof Promise)
        finale(_.extend(self, { _state: 3, _value: val }));
